...
OutputXdmf::OutputXdmf( const InputParameters &parameters )
...
void OutputXdmf::output( const ExecFlagType &type ){
    /*!
...
void OutputXdmf::writeMeshToFile( const bool local ){
    /*!
...
//        grid->insert( reference_grid->getAttribute( "SUBDOMAINID" ) );
    ...
    for ( auto & node : as_range( local ? _mesh.localNodesBegin() : _mesh.getMesh().nodes_begin(),
                                  local ? _mesh.localNodesEnd() : _mesh.getMesh().nodes_end() ) ){
    ...
    //Save the nodesets
    const std::set< BoundaryID > nodesetIds = _mesh.meshNodesetIds();
    ...
    for ( auto it = nodesetIds.begin(); it != nodesetIds.end(); it++ ){
        ...
        for ( auto nit = _mesh.getMesh().bid_nodes_begin( *it ); nit != _mesh.getMesh().bid_nodes_end( *it ); nit++ ){
            Node* node = *nit;
        ...
        XDMFnodeSets[ indx ]->setName( _mesh.getBoundaryName( *it ) );
    ...
    for ( auto & elem : as_range( local ? _mesh.activeLocalElementsBegin() : _mesh.getMesh().active_elements_begin(),
                                  local ? _mesh.activeLocalElementsEnd() : _mesh.getMesh().active_elements_end() ) ){
...
//    grid->insert( subdomainIds );
    ...
    const std::set< SubdomainID > subdomainIds = _mesh.meshSubdomains();
...
//        std::cout << _mesh.getSubdomainName( *it ) << ": " << *it  << "\n";

        //Get the size of the nodeset
        for ( auto eit =  _mesh.getMesh().active_subdomain_elements_begin( *it );
                   eit != _mesh.getMesh().active_subdomain_elements_end( *it ); eit++ ){
            Elem* elem = *eit;
        ...
        XDMFelementSets[ indx ]->setName( _mesh.getSubdomainName( *it ) );
...
void OutputXdmf::outputNodalVariables( const std::set< std::string > * system_names ){
    /*!
    ...
    _es.build_variable_names( names, nullptr, system_names );
    ...
    std::cout << "outputNames.size( ): " << outputNames.size( ) << "\n";

    //Determine the total number of variables and nodes
    unsigned int num_vars = names.size();
    unsigned int num_nodes = _mesh.getMesh().n_nodes();
    ...
    _es.build_solution_vector( soln, system_names );
...
const std::set< std::string > &OutputXdmf::getNodalVariableOutput( ){
    /*!
     * Get the nodal variables identified to be output
     */
